...
# Customizations from base image...

# Install xterm, latest versions of RN need it
RUN sudo apt-get update
RUN sudo apt-get install -qy xterm

# Install watchmen
RUN sudo apt-get install -qy python-dev
RUN sudo su -c "cd /tmp && git clone https://github.com/facebook/watchman.git && cd watchman && git checkout v4.1.0 && ./autogen.sh && ./configure && make && make install"
